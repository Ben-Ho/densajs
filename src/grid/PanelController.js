Ext.define('Densa.grid.PanelController', {
    ...
    init: function()
    {
        ...
        grid.on('beforeselect', function(sm, record) {
            ...
            Ext4.each(parentSessionView.query("[session]"), function(i) {
                if (i.getSession().getChangesForParent()) {
                    isDirty = true;
                }
            }, this);
            Ext4.each(parentSessionView.query("[controller]"), function(i) {
...
//             Ext4.each(sessionView.query("[session]"), function(i) {
